__author__ = 'Ryan Perkins and Matthew Fountain'
...
def greeting():
    greet = raw_input("Welcome to Hangman!  We hope you'll hang out here until your dying days.  Press Enter to begin.")
    return greet


#function to randomly choose a word from a prescribed list.
def generator():
    possible_words = ["cat", "dog", "mouse", "obstreperous", "crazy", "dogmatic"]
...
#of strikes.  The more wrong guesses results in more of the hangman being revealed.
...
def graphics(fail):
    ...
    if len(fail) == 1:
      ...
      #  return choose()
        ...
        print "Sorry you died.  Hang around or hang up your boots?"
        replay()
...
#Greeting is called, then an empty list for strikes to be added to later.  Strikes currently equals zero.  Generator function
#is called, and within this function, secret is the generator.  A ternary expression is used to turn each character
#of the generated word into an underscore.  The new list of underscores is called the mask.
...
#against the secret word and is correct, and within the if statement "true", print "good guess", print the new mask
    #with the revealed letters.  Then there's a conditional statement for when there are no blanks left, we
    #congratulate the winner and ask if he wants to play again.  If the check function comes up false, we add a strike
    # then append that strike to a list of strikes.  Then we call the graphics function with an argument for the number
    #of total strikes.  Then we call the graphics function with an argument for the number of total_strikes.
    #Then we say "bad guess" and print the mask again.
    ...
    while strikes < 5:
        ...
        if check(guess=guess, secret_word=secret, mask=mask):
            ...
            if mask.__contains__('_ '):
                ...
                print "Congratulations! You have escaped the Hangman."
                replay()
